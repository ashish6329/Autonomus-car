...
def weighted_img(img, initial_img, al=0.8, be=1.0, ga=0.0):
    """
    Returns resulting blend image computed as follows:
    initial_img * al + img * be + ga
    """
    img = np.uint8(img)
...
def color_frame_pipeline(frames, solid_lines=True, temporal_smoothing=True):
    ...
    img_color = frames[0] if is_videoclip else frames[0]
    img_blend = weighted_img(img_masked, img_color, al=0.8, be=1.0, ga=0.0)
